							...
							<form id="checkout_form" action="checkout_process.php" method="POST" class="was-validated">
								...
								<div class="row-checkout">
									...
									<div class="col-50">
										...
										<input type="text" id="fname" class="form-control" name="u_name" pattern="^[a-zA-Z ]+$"  value="'.$row["u_name"].'">
								...
								while($i<=$total_count){
									...
									$total=$total+$amount_ ;
									$sql = "SELECT p_id FROM products WHERE p_name='$item_name_'";
									$query = mysqli_query($con,$sql);
									$row=mysqli_fetch_array($query);
									$p_id=$row["p_id"];
									echo "	
									<input type='hidden' name='prod_id_$i' value='$p_id'>
